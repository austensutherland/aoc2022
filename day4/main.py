...
def part_one():
    ...
    for first_pair, second_pair in pairs:
        ...
        if any(item == [str(min_first_id), str(max_second_id)] for item in [first_pair, second_pair]):
            overlapping_items += 1
...
def part_two():
    ...
    overlapping_items = 0
    for first_pair, second_pair in pairs:
        first_pair = [int(first_pair[0]) + i for i in range((int(first_pair[1]) - int(first_pair[0])) + 1)]
        second_pair = [int(second_pair[0]) + i for i in range((int(second_pair[1]) - int(second_pair[0])) + 1)]
